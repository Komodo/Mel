...
"""A Code Intelligence Language Engine for the Mel language.
...
Module Usage:
    from cile_mel import scan
    mtime = os.stat("bar.mel")[stat.ST_MTIME]
    content = open("bar.mel", "r").read()
    scan(content, "bar.mel", mtime=mtime)
"""
...
log = logging.getLogger("cile.mel")
...
def scan_buf(buf, mtime=None, lang="Mel"):
    """Scan the given MelBuffer return an ElementTree (conforming
    to the CIX schema) giving a summary of its code elements.
    
    @param buf {MelBuffer} is the Mel buffer to scan
